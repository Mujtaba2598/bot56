...
app.use(express.static(path.join(__dirname)));  // serves static files (index.html)

// ==================== HEALTH CHECK (for Render) ====================
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', message: 'Halal AI Trading Bot - Real Trading' });
});
...
class AggressiveAITradingEngine {
    async analyzeAndTrade(sessionId, userEmail, apiKey, secretKey, config) {
        ...
        try {
            ...
            // Simulate profit (in real system you'd track entry & exit)
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    ...
    // Aggressive trading every 30 seconds
    const tradeInterval = setInterval(async () => {
        ...
        await aiEngine.analyzeAndTrade(
            sessionId,
            req.user.email,
            apiKey,
            secretKey,
